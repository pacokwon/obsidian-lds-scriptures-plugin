...
export async function fetchGenConTalk(url: string, method: 'GET' | 'POST' | 'PATCH'): Promise<GenConTalkData> {
  ...
  if (parsedData.pathParts[1] !== "general-conference") {
    throw new Error('This can only reference talks from General Conference.');
  }
  ...
  try {
    ...
    title = response.json.meta.title ? response.json.meta.title : "Title Not Found." // no need to search, the JSON holds the name. 
    ...
    if (parsedData.paragraphs) {
      ...
      if (typeof start === 'number' && Number.isInteger(start) && typeof paragraphEnd === 'number' && Number.isInteger(paragraphEnd)) {
        ...
        const endId = typeof paragraphEnd === 'string' ? paragraphEnd : `p${paragraphEnd}`;
        let collecting = false;
        $('.body-block').find('p, h1, h2, h3, h4, h5, h6').each((_, el) => {
          const $el = $(el);
